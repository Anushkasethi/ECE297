...
struct mouse_pan {
  ...
  double prev_y = 0;
  /**
   * The starting x and y positions of the mouse pointer during a panning event
   */
  double start_x = 0;
  double start_y = 0;
} g_mouse_pan;
...
gboolean press_mouse(GtkWidget *, GdkEventButton *event, gpointer data)
{
  ...
  if(event->type == GDK_BUTTON_PRESS) {
    ...
    if(event->button == PANNING_MOUSE_BUTTON) {
      ...
      g_mouse_pan.start_x = event->x;
      g_mouse_pan.start_y = event->y;
    }
...
gboolean release_mouse(GtkWidget *, GdkEventButton *event, gpointer data)
{
  ...
  if(event->type == GDK_BUTTON_RELEASE) {
    ...
    if(event->button == PANNING_MOUSE_BUTTON) {
      ...
      if(event->x == g_mouse_pan.start_x && event->y == g_mouse_pan.start_y && application->mouse_press_callback != nullptr) {
        ezgl::point2d const widget_coordinates(event->x, event->y);
